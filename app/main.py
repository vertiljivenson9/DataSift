...
from app.ml.analyzer import router as ml_router
...
async def dashboard(user=Depends(auth.get_current_user)):
    ...
    return {
        ...
        "usage": {
            ...
            "usage_percentage": round((user.monthly_requests / user.request_limit) * 100, 2)
...
async def http_exception_handler(request: Request, exc: HTTPException):
    return JSONResponse(
        ...
        content={"error": True, "code": exc.status_code, "message": exc.detail, "timestamp": int(time.time())}
    )
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        ...
        content={"error": True, "code": 500, "message": "Internal server error", "timestamp": int(time.time())}
    )
