...
class Payment(Base):
    ...
    status = Column(String(50), default="pending")  # pending, completed, failed, cancelled, refunded
...
class AnalysisReport(Base):
    ...
    # Analysis results
    analysis_type = Column(String(50), default="full")  # quick, full
...
class ApiKeyLog(Base):
    """API key usage logs for audit"""
    __tablename__ = "api_key_logs"
    
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    api_key_prefix = Column(String(10))
    action = Column(String(50))  # created, revoked, regenerated
